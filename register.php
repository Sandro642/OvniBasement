...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["confirm_password"]))){
        $confirm_password_err = "S'il vous plait confirmer votre mot de passe";     
    } else{
        $confirm_password = trim($_POST["confirm_password"]);
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "Les deux mot de passe ne correspondent pas";
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Ovni Basement | Sign Up</title>
...
<body>
    <div class="wrapper">
        <h2>Ovni Basement</h2>
        <p>Création de compte à Ovni Basement</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="reset" class="btn btn-secondary ml-2" value="Réinitialisé">
            </div>
            <p>Vous aviez déjà un compte ? <a href="login.php">Connection ICI</a>.</p>
        </form>
    </div>
      <script>
      function onClick(e) {
